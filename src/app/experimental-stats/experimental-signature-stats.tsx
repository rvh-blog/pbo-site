...
const moveUses = (appearance: EnrichedAppearance) => appearance.moveDataRecorded
  ...
  : null;
const favorableEvents = (appearance: EnrichedAppearance) => {
  const values = [appearance.favorableCrits, appearance.favorableMisses, appearance.favorableFlinches, appearance.favorableParalysis, appearance.favorableFreezes, appearance.favorableBurns, appearance.favorableSleep];
...
export function SignatureStatsReport({ appearances, minimumAppearances }: { appearances: EnrichedAppearance[]; minimumAppearances: number }) {
  ...
  return <section className="space-y-4">
    ...
    <div className="poke-card p-5 md:p-6">
      <div className="mb-4"><h3 className="font-pixel text-xs text-white">Normalized Pokémon signatures</h3><p className="mt-1 text-[10px] leading-4 text-[var(--foreground-muted)]">Damage per active turn and healing per active turn use shared appearances where both fields are saved. Damage per move, setup rate, and favorable-event rate use recorded move attempts. No missing values are imputed.</p></div>
      {rows.length ? <div className="overflow-x-auto rounded-xl border border-[var(--border)]"><table className="w-full min-w-[1100px] text-xs"><thead className="bg-[var(--background)] text-[9px] uppercase text-[var(--foreground-muted)]"><tr><th className="p-3 text-left">Pokémon</th><th>Season(s)</th><th>Apps</th><th>Record</th><th>DMG / active turn</th><th>DMG / move</th><th>KOs / 10 turns</th><th>Healing / active turn</th><th>Setup rate</th><th>Favorable event rate</th></tr></thead><tbody>{rows.map((row) => <tr key={row.id} className="border-t border-[var(--border)]"><td className="p-3"><div className="font-bold text-white">{row.name}</div><div className="text-[9px] text-[var(--foreground-muted)]">{row.spriteUrl ? "Sprite available" : "No sprite"}</div></td><td className="max-w-40 text-center text-[10px]">{row.seasons.join(", ")}</td><td className="text-center font-mono">{row.appearances}</td><td className="text-center font-mono">{row.wins}-{row.appearances - row.wins}</td><MetricCell metric={row.damagePerTurn} /><MetricCell metric={row.damagePerMove} /><MetricCell metric={row.kosPerTenTurns} /><MetricCell metric={row.healingPerTurn} /><MetricCell metric={row.setupRate} multiplier={100} digits={1} suffix="%" /><MetricCell metric={row.favorableEventRate} multiplier={100} digits={1} suffix="%" /></tr>)}</tbody></table></div> : <p className="text-xs text-[var(--foreground-muted)]">No Pokémon meet the current appearance threshold and filters.</p>}
    </div>
