...
export function ExperimentalStatsClient({ dataset, initialModule = "pokemon", initialFilters, standalone = false }: { dataset: ExperimentalStatsDataset; initialModule?: ExperimentalClientModule; initialFilters?: ExperimentalFilterState; standalone?: boolean }) {
  ...
  const protocolEventCount = filteredMatches.reduce((sum, match) => sum + match.battleEvents.length, 0);
    ...
    <div className="experimental-stats-readable readable-content space-y-6">
      ...
      <div className={standalone ? "" : "grid gap-6 xl:grid-cols-[230px_minmax(0,1fr)]"}>
        ...
        <main className="min-w-0 space-y-6">
          {module !== "glossary" ? <div className="grid grid-cols-2 gap-3 md:grid-cols-5">
          ...
          {module === "leaderboard" && <ExpandedLeaderboardModule rows={leaderboardEntity === "pokemon" ? pokemonRows : coachRows} entity={leaderboardEntity} setEntity={setLeaderboardEntity} perAppearance={leaderboardRate} setPerAppearance={setLeaderboardRate} />}
          {module === "replays" && <ReplaySearchModule matches={filteredMatches} />}
          {module === "visualizer" && <><div className="space-y-2"><BattleVisualizer matches={filteredMatches} selectedId={visualMatchId} onSelect={setVisualMatchId} /><div className="poke-card px-4 py-2 text-center text-[10px] font-bold uppercase tracking-wide text-[var(--foreground-muted)]">HP chart — X axis: Turn · Y axis: Team HP remaining (%) · Item reveal chart — X axis: Turn range of first reveal · Y axis: Item reveal count</div></div><EventAnalytics matches={filteredMatches} selectedId={visualMatchId} /></>}
...
function BattleVisualizer({ matches, selectedId, onSelect }: { matches: ExperimentalMatch[]; selectedId: number | null; onSelect: (id: number) => void }) {
  const eligible = matches.filter((match) => match.turnSnapshots.length > 0 || match.battleEvents.length > 0);
...
function ExpandedRollingModule({ pokemon, appearances, rows, onSelect }: { pokemon: EntityAggregate | null; appearances: EnrichedAppearance[]; rows: EntityAggregate[]; onSelect: (id: number) => void }) {
  ...
  const trendRows = [
    { metric: "Damage / appearance", previous: average(previous, totalDamage, (a) => a.damageDealt !== null), latest: average(latest, totalDamage, (a) => a.damageDealt !== null) },
...
function ExpandedLeaderboardModule({ rows, entity, setEntity, perAppearance, setPerAppearance }: { rows: EntityAggregate[]; entity: "pokemon" | "coach"; setEntity: (value: "pokemon" | "coach") => void; perAppearance: boolean; setPerAppearance: (value: boolean) => void }) {
  ...
  type SortKey = "name" | "appearances" | "wins" | "value" | "coverage";
  const [sortKey, setSortKey] = useState<SortKey>("value");
  const [sortDirection, setSortDirection] = useState<"asc" | "desc">("desc");
  const coverageFor = (row: EntityAggregate) => metric === "damage" ? row.damageAppearances : metric === "healing" ? row.healingAppearances : metric === "turns" ? row.turnsAppearances : metric === "setup" ? row.setupAppearances : metric === "favorable" ? row.eventAppearances : row.appearances;
  const sortValue = (row: EntityAggregate): string | number | null => sortKey === "name" ? row.name : sortKey === "appearances" ? row.appearances : sortKey === "wins" ? row.wins : sortKey === "coverage" ? coverageFor(row) : valueFor(row);
  ...
  const changeSort = (nextKey: SortKey) => {
    ...
    else { setSortKey(nextKey); setSortDirection(nextKey === "name" ? "asc" : "desc"); }
  };
  const sortableHeader = (label: string, key: SortKey, className = "") => <th className={className}><button type="button" onClick={() => changeSort(key)} className="inline-flex items-center gap-1 font-inherit text-inherit hover:text-white" title={`Sort by ${label}`}>{label}<span aria-hidden="true" className="text-[10px] text-violet-300">{sortKey === key ? sortDirection === "asc" ? "↑" : "↓" : "↕"}</span></button></th>;
...
function EventRareRecords({ matches }: { matches: ExperimentalMatch[] }) {
  ...
  const mostSwitches = [...rows].sort((a, b) => b.switches - a.switches)[0];
  const mostTera = [...rows].sort((a, b) => b.tera - a.tera)[0];
