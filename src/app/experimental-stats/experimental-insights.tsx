...
export function ExperimentalInsights({ matches, appearances }: { matches: ExperimentalMatch[]; appearances: InsightAppearance[] }) {
  ...
  const momentumSelectWidth = Math.min(560, Math.max(320, ...momentumMatches.map((match) => (`${match.coach1.teamName} vs ${match.coach2.teamName} · ${match.seasonName} · ${match.divisionName} · W${match.week}`).length * 7 + 48), 320));
  ...
  return <section className="space-y-4">
    <div className="poke-card border-cyan-400/25 bg-cyan-500/[0.05] p-5 md:p-6"><h2 className="font-pixel text-sm text-white">How to use Insights</h2><p className="mt-2 max-w-3xl text-xs leading-5 text-[var(--foreground-muted)]">This tab summarizes patterns found in the replays selected by your filters. It describes what happened in those battles; it does not grade decisions or predict future results.</p><div className="mt-4 grid gap-3 md:grid-cols-3"><div className="rounded-lg border border-[var(--border)] bg-[var(--background)] p-3"><strong className="text-xs text-cyan-200">1. Start with coverage</strong><p className="mt-1 text-[10px] leading-4 text-[var(--foreground-muted)]">The badges show how many replays contain the data needed for each metric.</p></div><div className="rounded-lg border border-[var(--border)] bg-[var(--background)] p-3"><strong className="text-xs text-violet-200">2. Read the overview</strong><p className="mt-1 text-[10px] leading-4 text-[var(--foreground-muted)]">The first cards summarize faint order, comebacks, battle pace, and saved items.</p></div><div className="rounded-lg border border-[var(--border)] bg-[var(--background)] p-3"><strong className="text-xs text-emerald-200">3. Explore the evidence</strong><p className="mt-1 text-[10px] leading-4 text-[var(--foreground-muted)]">Charts and tables break those summaries down; each table includes a plain-language reading guide.</p></div></div></div>
    {addedReports}
