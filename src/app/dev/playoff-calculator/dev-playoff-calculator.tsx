      ...
      <div className="border-t border-[var(--background-tertiary)] p-4 sm:p-5">
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          ...
          <article className="rounded-xl border border-[var(--background-tertiary)] bg-[var(--background)] p-4">
            ...
            <p className="mt-2 text-sm leading-relaxed text-[var(--foreground-muted)]"><strong className="text-white">Neutral</strong> gives both teams an equal chance. <strong className="text-white">Standings</strong> uses league results and recent play. <strong className="text-white">ELO</strong> uses coach ELO ratings when available.</p>
          </article>
          <article className="rounded-xl border border-[var(--background-tertiary)] bg-[var(--background)] p-4">
...
export function DevPlayoffCalculator({ seasonName, divisions, teams, matches, demoDivisionIds = [] }: { seasonName: string; divisions: CalculatorDivision[]; teams: CalculatorTeam[]; matches: CalculatorMatch[]; demoDivisionIds?: number[] }) {
  ...
  const [predictionModel, setPredictionModel] = useState<PredictionModel>("standings");
  ...
  const workerAnalysis = usePlayoffWorker({ teams: divisionTeams, matches: divisionMatches, predictions, model: predictionModel, teamId: effectiveSelectedTeamId, analysisEnabled, primaryMatchId: primaryOpenMatchId });
  ...
  return (
    <div className="mx-auto max-w-[1600px] space-y-5 pb-12">
      <header className="poke-card overflow-hidden">
        <div className="border-b border-red-400/20 bg-red-400/10 px-5 py-2 text-[10px] font-black uppercase tracking-[0.2em] text-red-300">Development only · Nothing is written to league data</div>
      ...
      <div className="grid items-start gap-5 xl:grid-cols-[minmax(420px,0.9fr)_minmax(680px,1.3fr)]">
        <section className="poke-card p-4 sm:p-5 xl:sticky xl:top-4"><div className="mb-4"><p className="text-[10px] font-bold uppercase tracking-widest text-[var(--primary)]">Scenario editor</p><h2 className="mt-1 text-xl font-black text-white">Match outcomes</h2></div>
          <div className="mb-3 grid gap-2 sm:grid-cols-2"><select value={weekFilter} onChange={(event) => setWeekFilter(event.target.value === "all" ? "all" : Number(event.target.value))} className="rounded bg-[var(--background)] px-2 py-2 text-xs text-white"><option value="all">All weeks</option>{weeks.map((week) => <option key={week} value={week}>Week {week}</option>)}</select><select value={teamFilter} onChange={(event) => setTeamFilter(event.target.value === "all" ? "all" : Number(event.target.value))} className="rounded bg-[var(--background)] px-2 py-2 text-xs text-white"><option value="all">All teams</option>{activeTeams.map((team) => <option key={team.id} value={team.id}>{team.teamName}</option>)}</select><select value={matchFilter} onChange={(event) => setMatchFilter(event.target.value as MatchFilter)} className="rounded bg-[var(--background)] px-2 py-2 text-xs text-white"><option value="all">All statuses</option><option value="picked">Edited</option><option value="unpicked">Unedited</option><option value="locked">Locked</option></select><select value={differentialMode} onChange={(event) => setDifferentialMode(event.target.value as DifferentialMode)} className="rounded bg-[var(--background)] px-2 py-2 text-xs text-white"><option value="manual">Manual +3 default</option><option value="typical">Team average diff</option><option value="probability">Modeled diff</option></select><select value={predictionModel} onChange={(event) => setPredictionModel(event.target.value as PredictionModel)} className="rounded bg-[var(--background)] px-2 py-2 text-xs text-white"><option value="neutral">Neutral 50/50 model</option><option value="standings">Standings-based model</option><option value="elo">ELO-based model</option></select><button type="button" onClick={() => { const nextWeek = Math.min(...remainingMatches.map((match) => match.week)); if (Number.isFinite(nextWeek)) setWeekFilter(nextWeek); setShowCompleted(false); setMatchFilter("all"); }} className="rounded border border-red-400/30 bg-red-400/10 px-2 py-2 text-xs font-bold text-red-300">Live week mode · {liveMatchIds.size} active</button></div>
