...
export function OverlayV1Client({ data, battleUrl, context, multiCast = false }: Props) {
  ...
  const [footerTab, setFooterTab] = useState<"log" | "context" | "calc">("context");
  ...
  return createPortal(
    ...
    >
      ...
      {/* ── No background needed - battle scene is fullscreen ── */}

      {/* ── No branding wash/dots/scanlines needed in fullscreen mode ── */}
      ...
      {/* ── No viewport border needed in fullscreen mode ── */}

      {/* ── TOP HEADER: SCOREBOARD ── */}
      {!hideUI && <div className="absolute z-50" style={{ right: 337, width: 600, bottom: 1080 - 824 + 8 }}>
        <Scoreboard
          leftTeam={leftTeam}
          rightTeam={rightTeam}
          leftAlive={leftAlive}
      ...
      {/* ── Sidebar backgrounds ── */}
      <div className="absolute z-[6] pointer-events-none" style={{ top: 0, left: 0, width: 325, bottom: 0 }}>
        <div className="absolute inset-0 bg-[#020617]/75" />
        <div className="absolute inset-0 bg-dots opacity-30" />
        <div className="absolute inset-0 scanlines opacity-15" />
        <div className="absolute top-0 bottom-0 right-0 w-[2px]" style={{ backgroundColor: divisionColor }} />
      </div>
      <div className="absolute z-[6] pointer-events-none" style={{ top: 0, right: 0, width: 325, bottom: 0 }}>
        <div className="absolute inset-0 bg-[#020617]/75" />
        <div className="absolute inset-0 bg-dots opacity-30" />
        <div className="absolute inset-0 scanlines opacity-15" />
        <div className="absolute top-0 bottom-0 left-0 w-[2px]" style={{ backgroundColor: divisionColor }} />
      </div>

      {/* ── SIDEBAR LEFT: TEAM 1 ── */}
      <div className="absolute z-30 px-4 flex flex-col overflow-visible" style={{
        top: 130,
        left: 0,
        width: 325,
        bottom: 1080 - 824 + 8,
      ...
      {/* ── FOOTER: SINGLE TABBED PANEL ── */}
      {!hideUI && <div className="absolute z-20" style={{
        width: 600,
        right: 337,
        top: 824,
        bottom: 12,
      }}>
        <div className="h-full rounded-xl border flex flex-col overflow-hidden shadow-2xl relative" style={{ borderColor: `${divisionColor}44` }}>
          <div className="absolute inset-0 bg-[#020617]/75 rounded-xl" />
          <div className="absolute inset-0 bg-dots opacity-30 rounded-xl" />
          <div className="absolute inset-0 scanlines opacity-15 rounded-xl" />
          <div className="absolute inset-0 bg-slate-950/70 rounded-xl" />
          {/* Tab Header */}
          <div className="relative z-10 flex items-center px-3 py-2 border-b border-white/5 shrink-0 gap-1">
            <button
              onClick={() => setFooterTab("context")}
              className={`flex items-center gap-1.5 px-3 py-1 rounded text-[10px] font-bold uppercase tracking-widest transition-colors ${
                footerTab === "context" ? "text-white" : "text-slate-500 hover:text-slate-300"
              }`}
              style={footerTab === "context" ? { backgroundColor: `${divisionColor}33` } : undefined}
            >
              <Zap size={12} style={{ color: footerTab === "context" ? divisionColor : undefined }} />
              Context
            </button>
            <button
              onClick={() => setFooterTab("log")}
              className={`flex items-center gap-1.5 px-3 py-1 rounded text-[10px] font-bold uppercase tracking-widest transition-colors ${
                footerTab === "log" ? "text-white" : "text-slate-500 hover:text-slate-300"
              }`}
              style={footerTab === "log" ? { backgroundColor: `${divisionColor}33` } : undefined}
            >
              <ScrollText size={12} style={{ color: footerTab === "log" ? divisionColor : undefined }} />
              Battle Log
            </button>
            <button
              onClick={() => setFooterTab("calc")}
              className={`flex items-center gap-1.5 px-3 py-1 rounded text-[10px] font-bold uppercase tracking-widest transition-colors ${
                footerTab === "calc" ? "text-white" : "text-slate-500 hover:text-slate-300"
              }`}
              style={footerTab === "calc" ? { backgroundColor: `${divisionColor}33` } : undefined}
            >
              <Calculator size={12} style={{ color: footerTab === "calc" ? divisionColor : undefined }} />
              Damage Calc
            </button>
          </div>
          ...
          <div className="relative z-10 flex-1 min-h-0 overflow-hidden">
            <div style={{ display: footerTab === "log" ? "flex" : "none" }} className="h-full flex-col">
              <BattleLogPanel
            ...
            <div style={{ display: footerTab === "context" ? "block" : "none" }} className="h-full overflow-y-auto">
              <ContextPanel
                data={data}
                context={context}
                leftTeam={leftTeam}
      ...
      <div className="absolute z-20 flex flex-col px-4" style={{
        left: 0,
        top: 824,
        width: 325,
        bottom: 12,
      }}>
        <div className="flex-1 bg-slate-950/70 backdrop-blur-xl rounded-xl border overflow-hidden shadow-2xl flex flex-col" style={{ borderColor: `${divisionColor}44` }}>
      ...
      {/* ── GLOBAL FRAME ── */}
      {/* border frame removed for fullscreen mode */}
...
function Scoreboard({
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-slate-950/70 rounded-xl" />
      {/* Status bar */}
      <div className="relative z-10 flex items-center justify-between px-3 py-1.5 border-b border-white/5">
        <div className="flex items-center gap-2 text-[10px] font-bold tracking-[0.15em] text-slate-400 uppercase">
          {battleEnded ? (
      ...
      <div className="relative z-10 flex items-center justify-between px-4 py-2">
        ...
        <div className="flex items-center gap-3">
          <span className="text-xl font-black italic opacity-50 text-white">{t1abbr}</span>
          <div className="h-8 w-0.5 rounded-full" style={{ backgroundColor: divisionColor }} />
          <div className="flex flex-col">
            <span className="text-[9px] font-bold text-slate-500 uppercase">ALIVE</span>
            <span className="text-2xl font-black text-white leading-none tracking-tighter" style={{ fontFamily: "'Press Start 2P', cursive" }}>
              {leftAlive}
            </span>
          </div>
        </div>
        ...
        {/* Center clock */}
        <div className="text-base font-black text-white uppercase italic tracking-wider px-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="h-8 w-0.5 rounded-full" style={{ backgroundColor: divisionColor }} />
          <span className="text-xl font-black italic opacity-50 text-white">{t2abbr}</span>
        </div>
      </div>
    </div>
  );
...
function TeamHeader({
  ...
  return (
    <div className={`relative flex flex-col justify-end ${isLeft ? "items-start" : "items-end"} mb-2 h-[100px]`}>
...
function ContextPanel({
  ...
  return (
    <div className="flex-1 flex flex-col relative overflow-hidden">
      ...
      <div className="flex-1 flex flex-col gap-1 min-h-0 px-4 pt-1 pb-1">
          ...
          <div className="flex-1 flex flex-col gap-0.5">
            ...
            <div className="flex items-center gap-1">
              ...
              <select value={flipped ? defNature : atkNature} onChange={(e) => flipped ? setDefNature(e.target.value) : setAtkNature(e.target.value)}
                ...
                {natures.map((n) => <option key={n.name} value={n.name}>{n.label}</option>)}
              </select>
            </div>
            <div className="flex items-center gap-1">
...
function BattleLogPanel({
  ...
  return (
    <div
      className="flex-1 overflow-y-auto overflow-x-hidden p-3 battle-log-container"
      ref={containerRef}
      style={{ ["--division-color" as string]: divisionColor }}
    />
  );
}
