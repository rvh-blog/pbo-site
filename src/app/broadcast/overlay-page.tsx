...
import { matches, transactions, seasonCoaches } from "@/lib/schema";
...
import { type OverlayData } from "./overlay/overlay-client";
import { OverlayV1Client } from "./overlay/overlay-v1-client";
import { Overlay2Client as OverlayV2Client } from "./overlay2/overlay2-client";
import type { MatchContext } from "./overlay2/overlay2-client";
...
import { formatPokemonDisplayName } from "@/lib/pokemon-name-utils";
...
export async function BroadcastOverlayPage({ searchParams, variant }: BroadcastOverlayPageProps) {
  ...
  const seasonRules = getSeasonBattleRules(match.season?.seasonNumber);
  ...
  const [roster1Result, roster2Result, aliasMaps] = await Promise.all([
    getTimeSyncedRoster(
      ...
      match.week,
      coach1?.rosters || [],
      [...coach1Txs[0], ...coach1Txs[1]] as TimeSyncTransaction[]
    ),
    getTimeSyncedRoster(
      match.coach2SeasonId,
      match.week,
      coach2?.rosters || [],
      [...coach2Txs[0], ...coach2Txs[1]] as TimeSyncTransaction[]
    ),
    getPokemonAliasMaps(),
  ]);
  ...
  function buildRoster(result: typeof roster1Result) {
    return [
      ...result.filteredRosters.map((r) => ({
        ...
        name: r.pokemon?.name || "",
        displayName: formatPokemonDisplayName(
          r.pokemon?.name,
          r.pokemon?.displayName,
          { friendlyMegaNames: seasonRules.friendlyMegaNames }
        ),
  ...
  const h2h = { coach1Wins: 0, coach2Wins: 0 };
