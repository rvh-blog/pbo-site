...
function parseDraftCSV(divisionName: string): TeamData[] {
  const filePath = `./data/S7/${divisionName} S7 - Draft.csv`;
...
function parseMatchCSV(divisionName: string): MatchData[] {
  const filePath = `./data/S7/${divisionName} S7 - Match Stats.csv`;
  const content = fs.readFileSync(filePath, "utf-8");
  const lines = content.split("\n").map((l) => l.split(","));
...
// Collect prices from all draft CSVs instead of parsing Tiers CSV (S7 has different tier format)
function collectPricesFromDrafts(): PriceInfo[] {
  const prices: PriceInfo[] = [];
  const seenPokemon = new Map<string, PriceInfo>();
    ...
    for (const group of rowGroups) {
      for (const offset of teamOffsets) {
        for (let i = 0; i < 12; i++) {
          ...
          const tcMarker = row[offset + 4]?.trim();
          const isTeraCaptain = tcMarker === "T";
          const tcCost = isTeraCaptain ? (parseInt(tcCostStr || "0") || 0) : null;

          const normalizedKey = pokeName.toLowerCase();

          // Track TC cost if this pick is a TC
          if (!seenPokemon.has(normalizedKey)) {
            seenPokemon.set(normalizedKey, {
              name: pokeName,
              price,
              teraCaptainCost: tcCost,
            });
          } else if (tcCost !== null && seenPokemon.get(normalizedKey)!.teraCaptainCost === null) {
            // Update TC cost if we found one
            seenPokemon.get(normalizedKey)!.teraCaptainCost = tcCost;
          }
        }
      }
    }
  ...
  return Array.from(seenPokemon.values());
}

async function seedS7() {
  console.log(`\n========================================`);
  console.log(`Seeding ${SEASON_NAME}...`);
  console.log(`========================================\n`);

  // Check if Season 7 already exists
  let season7 = await db.query.seasons.findFirst({
  ...
  console.log(`${SEASON_NAME} ID: ${season7.id}`);

  // Parse and seed Pokemon prices from draft data
  console.log("\nCollecting Pokemon prices from Draft CSVs...");
  const tierPrices = collectPricesFromDrafts();
  ...
  // Check existing S7 prices
  const existingS7Prices = await db.query.seasonPokemonPrices.findMany({
    where: eq(schema.seasonPokemonPrices.seasonId, season7.id),
  });

  if (existingS7Prices.length === 0) {
    ...
    console.log(`${SEASON_NAME} already has ${existingS7Prices.length} Pokemon prices`);
  }
  ...
  for (let divIdx = 0; divIdx < DIVISIONS.length; divIdx++) {
    ...
    const draftFile = `./data/S7/${divisionName} S7 - Draft.csv`;
    const matchFile = `./data/S7/${divisionName} S7 - Match Stats.csv`;
    ...
    let division = await db.query.divisions.findFirst({
      where: and(
        eq(schema.divisions.seasonId, season7.id),
    ...
    if (!division) {
      ...
      const [newDiv] = await db
        ...
        .values({ seasonId: season7.id, name: divisionName })
        .returning();
      division = newDiv;
    }
