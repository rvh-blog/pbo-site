#!/usr/bin/env python3
"""
Parse S8 CSV match stats to verify forfeits match DB.
"""
...
DATA_DIR = Path("/Users/rafu/pbo-site/data/S8")

DIVISION_CSV = {
    "Neon": "Neon S8 - Match Stats.csv",
    "Stargazer": "Stargazer S8 - Match Stats.csv",
    "Sunset": "Sunset S8 - Match Stats.csv",
}
...
def find_match_in_csv(rows, week, team1, team2):
    ...
    for i, row in enumerate(rows):
        ...
        # Check for match (fuzzy - any team name contains the search term)
        match_found = False
        ...
        # Check for Abra placeholder in Pokemon rows
        has_abra = False
        has_real_data = False
        for j in range(2, 8):
            if i + j >= len(rows):
...
def main():
    ...
    print(f"Found {len(missing_matches)} S8 matches missing Pokemon data\n")
