...
const TEAM_NAME_ALIASES: Record<string, string> = {
  ...
  "charleston chesnaught's": "Charleston Chestnaughts",
  "charleston chestnaughts": "Charleston Chestnaughts",
  "charleston chesnaughts": "Charleston Chestnaughts",

  // New York Malamar variations
  "new york malamar": "New York Malamars",
  "new york malamars": "New York Malamars",

  // Worcester variations
  "worcester woopers": "Worcester Woopers",
  "worchester woopers": "Worcester Woopers",
  "worcestor woopers": "Worcester Woopers",

  // Virginia Zekrom variations
  "virginia zekrom's": "Virginia Zekroms",
  "virginia zekroms": "Virginia Zekroms",

  // Sunnyside Suicune variations
  "sunnyside suicune's": "Sunnyside Suicunes",
  "sunnyside suicunes": "Sunnyside Suicunes",

  // Philadelphia shortened
  "philadelphia": "Philadelphia Pinsirs",
  "philadelphia pinsirs": "Philadelphia Pinsirs",

  // Marble Falls (appears to have replaced a team in Week 2)
  "marble falls miniors": "Abbotsford Aggrons", // Seems like a data entry error - this match result doesn't make sense
};
...
async function main() {
  console.log("Importing S4 Unova schedule...\n");

  // 1. Get S4 season and Unova division
  ...
  console.log(`Found S4 (id: ${season.id}) with Unova division (id: ${unovaDivision.id})`);

  // 2. Parse the schedule CSV
  const csvPath = path.join(__dirname, "../data/S4/PBO S4 UNOVA DOC - S4 SCHEDULE_Match Replays.csv");
  ...
  for (let i = 27; i < lines.length; i++) {
    ...
    // Skip weird data (like Marble Falls vs Virginia where both have +2)
    if (team1Result.isWinner && team2Result.isWinner) {
      console.log(`Skipping invalid match (both winners): ${team1Str} vs ${team2Str}`);
      continue;
    }
  ...
  if (existingMatches.length > 0) {
    console.log(`\n⚠️  Found ${existingMatches.length} existing matches for S4 Unova.`);
  ...
  await db.delete(schema.playoffMatches).where(
    and(
      ...
      eq(schema.playoffMatches.divisionId, unovaDivision.id)
    )
  );
  ...
  console.log("\n⚠️  Note: Playoffs are incomplete. Finals and some semi-finals data is missing.");
}
